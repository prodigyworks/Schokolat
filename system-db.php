		...
		if (! isset($_SESSION['SITE_CONFIG'])) {
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$data->refreshcycle = $member['refreshcycle'];
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@tclplanner.co.uk', 'Schokolat', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
...
function sendTeamMessage($id, $subject, $message, $footer = "", $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'confirmation@tclplanner.co.uk', 'Schokolat', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@tclplanner.co.uk', 'Schokolat', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
function getLoggedOnEventID() {
	start_db();
	
	if (! isset($_SESSION['SESS_EVENT_ID'])) {
		return 0;
	}
	
	return $_SESSION['SESS_EVENT_ID'];
}
